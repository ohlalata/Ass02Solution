        ...
        private void LoadCart()
        {
            try
            {
                ...
                if (cart == null || cart.Count == 0)
                {
                    ...
                    lableOrderTotal.Text = $@"Order Total: {0}";
                }
                else
                {
                    ...
                    foreach (var cartItem in cart)
                    {
                        CartPresenter cartPresenter = new CartPresenter
                        {
                            ...
                            Quantity = cartItem.Value.Quantity > productRepository.GetProduct(cartItem.Key).UnitsInStock ? productRepository.GetProduct(cartItem.Key).UnitsInStock : cartItem.Value.Quantity,
